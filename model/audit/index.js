...
const audit = {
    // 审核列表 -  查询 出来 所以 审核 - 通过 、未通过 、 和审核中的 数据，已发布的不审核列表中显示
    auditList: async ({ authorId, auditState, publishState }) => {
        ...
        const data = await promisePool.query(`select n.*, 
            ...
            where (auditState >= ${auditState}) and (n.authorId=${authorId}) and (newsDele != 1 )  and (publishState <= ${publishState} )   GROUP BY n.id; `);
        return data;

    },
    // 审核新闻  -  查询 出来 所有 auditState 审核状态等于1审核中的数据 - 这里 不会返回自己或者同级的 审核新闻数据  除非是 roleid 顶级权限 为1 超级管理员 返回所有 审核状态等于1的数据 
    // 并且  返回 小于当前 roleid (注意 roleid 顶级权限 为1 超级管理员 ，因此 大于roleid 权限就是小的权限 ) 权限的用户数据, 并且 roleid=1 查询所有区域的用户 否则返回  当前 级别 以下的同区域的用户
    auditNews: async ({ auditState,roleid,region }) => {
        const data = await promisePool.query(`select n.*, 
        IF( COUNT(c.id) = 0,JSON_ARRAY(), CAST(
        GROUP_CONCAT( JSON_OBJECT('id', c.id,
        'title', c.title,
